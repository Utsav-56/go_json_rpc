...
func (s *RpcServer) StartWithContext(ctx context.Context) error {
	...
	if s.Config.UseTcp {
		...
		ln, err = winio.ListenPipe(fmt.Sprintf(`\\.\pipe\%s`, s.Config.PipeName), nil)
		if err != nil {
			return fmt.Errorf("Failed to start named pipe listener: %v", err)
		}
		log.Printf("Server listening on named pipe: %s", s.Config.PipeName)
	}
